...
- Chore **(Major update)**: minimum required `eslint-plugin-ava` version
  is now `^4.2.0`.

- Chore **(Major update)**: minimum required `eslint-plugin-import` version
  is now `^2.3.0`.

- Chore **(Major update)**: minimum required `eslint-plugin-unicorn` version
  is now `^2.1.1`.

- Chore **(Major update)**: minimum required `eslint-plugin-react` version
  is now `^2.1.1`.

- Chore **(Major update)**: minimum required `eslint-plugin-promise` version
  is now `^3.5.0`.

- Chore **(Major update)**: minimum required `eslint-plugin-lodash` version
  is now `^2.4.2`.

- Chore **(Major update)**: minimum required `eslint-plugin-import` version
  is now `^2.6.0`.

- Chore **(Major update)**: minimum required `eslint-plugin-jsx-a11y` version
  is now `^6.0.0`.

- Chore **(Major update)**: minimum required `eslint-plugin-node` version
  is now `^5.1.0`.
...
- Removed: `prettier/prettier` preset in favor build-in `prettier` plugin
  in `core`, `es5`, `esnext`, `react` preset.

- Removed: `import/newline-after-import` rule
  in favor `padding-line-between-statements`.
...
- Changed: `react/jsx-wrap-multilines` require parentheses
  around arrow (functions and expressions).
...
- Added: `allowTaggedTemplates` with `true` value
  in `no-unused-expressions` rule.
...
- Changed: set `ignoreIndirectDependencies` to `true`
  in `node/no-hide-core-modules` rule.
...
- Changed: set `maximum` to `1` and `when` to `multiline`
  in `react/jsx-max-props-per-line` rule.
...
- Added: function `Immutable.*` in `capIsNewExceptions` option
  to `new-cap` rule.

- Added **(Major update)**: option `balanced: false` to `spaced-comment` rule
  in `block` section.

- Added **(Major update)**: `ArrayExpression` and `ObjectExpression` option
  with value `1` in `indent` rule.
...
- Added **(Major update)**: options `'asyncArrow': 'always'`
  to `space-before-function-paren` rule.
...
- Chore **(Major update)**: minimum required `babel-core` version
  is now `^6.17.0`.
...
- Chore **(Major update)**: update `ecmaVersion` from `7` to `8` 
  in `cssnext` preset.
...
- Added: `convertPath` options to `eslint-plugin-node` for good work
  with `babel`.
...
- Chore **(Major update)**: minimum required `eslint-plugin-xo` version
  is now `^1.0.0`.
...
- Chore: added `deprecated` rules for `eslint`, `eslint-plugin-import`
  for passed `eslint-find-rules` test.

- Chore: added `remarkConfig` section to `package.json` for 
  use own `remark-preset-lint-itgalaxy` preset.
...
- Changed: `FunctionDeclaration` and `FunctionExpression`
  to `{ "body": 1, "parameters": 1 }` in `indent` rule.
...
- Added: `experimentalObjectRestSpread` option to `parserOptions`
  in `esnext` bundle.
...
- Changed: `generator-star-spacing` now require no space before and
  space after `*`.
...
- Changed: `allowAfterThis` and `allowAfterSuper` to `false`
  in `no-underscore-dangle` rule.
...
- Chore: use `remark-preset-lint-itgalaxy` 
  instead `remark-lint-config-itgalaxy`.
...
- Chore: use `remark-lint-config-itgalaxy` sharable config 
  for linting `markdown`.

- Chore: sort alphabetically `devDependencies`.

- Chore: rename `LICENSE` to `LICENSE.md`.

- Chore: `files` section in `package.json` now contain `CHANGELOG.md` 
  and `README.md`.
...
- Chore **(Major update)**: update minimal version `eslint-plugin-ava`
  to `3.0.0`.
...
- Chore **(Major update)**: update minimal version `eslint-plugin-react`
  to `6.0.0`.
...
- Added: new version `eslint-plugin-jsx-a11y` and new rules 
  from `eslint-plugin-jsx-a11y` plugin.
